# herdr-agentchat

herdr 上で leader (Claude Code) と coder (Codex) が、人間の伝書鳩なしに会話しながら実装を進めるためのプラグイン。

中核は send 一体型の設計。`actions/send.sh` が宛先への本文投入と「相手を起こす」を
`herdr agent prompt --wait` の一手で行い、宛先の着手までをその場で見届ける。
そのため「未応答があるかの判定」や「起こす係」が不要になる。

## 構成

```
herdr-agentchat/
  herdr-plugin.toml     # マニフェスト
  actions/
    env.sh              # CLI・state directory・JSON 抽出の共通定義
    common.sh           # setup 系 action の共有関数
    list.sh             # CLI 疎通確認用 (agent list を返すだけ)
    send.sh             # 送信 + 起こす (中核)
    setup.sh            # leader / coder 用意 + ブリーフィング
    start-leader.sh     # leader の採用または起動
    start-coder.sh      # coder の起動
    verify-link.sh      # leader へのブリーフィング送信 + 着手確認
  events/
    log-status.sh       # agent 状態遷移を $HERDR_PLUGIN_STATE_DIR/status-events.log へ記録
    relay.sh            # agent 完了時の返信を pending sender へ中継
  templates/
    CLAUDE.md           # 2 体運用規約 (AGENTS.md はこれへのシンボリックリンク)
```

## 前提

- macOS、herdr 0.7.x
- Claude Code / Codex CLI がインストール・ログイン済み

## セットアップ

1. プラグインを登録する。

   ```bash
   herdr plugin install thkt/herdr-agentchat
   herdr plugin list
   ```

   ローカル開発時は clone したディレクトリを `herdr plugin link /path/to/herdr-agentchat` で登録する。
   更新の取り込みは `herdr plugin install` の再実行 (v1 に update コマンドはない)。
   install した実体 (send.sh の置き場所) は `~/.config/herdr/plugins/github/thkt.agentchat-*/` に入る。

2. 運用規約を配る。

   ```bash
   cp templates/CLAUDE.md <project>/CLAUDE.md
   ln -s CLAUDE.md <project>/AGENTS.md
   ```

3. プロジェクトディレクトリで herdr を起動し、対象 workspace にフォーカスした状態で
   セットアップ action を実行する。プロンプト解釈に委ねず決定論で 2 ペインが揃う。

   ```bash
   herdr plugin action invoke thkt.agentchat.setup   # leader / coder 用意 + briefing 着手確認
   ```

   setup は以下 3 action の一括実行で、個別にも呼べる。

   ```bash
   herdr plugin action invoke thkt.agentchat.start-leader   # claude 起動 + leader 命名
   herdr plugin action invoke thkt.agentchat.start-coder    # codex 起動 + coder 命名
   herdr plugin action invoke thkt.agentchat.verify-link    # ダイアログ解消 + briefing 送信・着手確認
   ```

   start-leader はフォーカスペインで claude がすでに動いていれば新規起動せず、
   その claude を rename で leader に採用する。採用したセッションは起動時規約を
   読んでいないため、役割は verify-link のブリーフィングで届く。

   各 action は、既存 agent が居る正常系では再実行可能。途中失敗時の rollback は行わない。
   実行結果は
   `herdr plugin log list --plugin thkt.agentchat` で確認する。
   起動引数を変えるときは `$(herdr plugin config-dir thkt.agentchat)/agentchat.conf` に
   `LEADER_ARGS` / `CODER_ARGS` を書く (既定: leader は素の claude、coder は
   `-a never -s workspace-write -c sandbox_workspace_write.network_access=true`)。

   verify-link は coder の初回ダイアログを解消し、leader にだけ役割と send コマンドの
   絶対パスをブリーフィングとして送って着手を確認する。応答の完了や内容は検証しない。
   leader の「委譲は send.sh で行う」だけは中継機構で代替できない (send.sh を経由しないと
   返信待ちマーカーが書かれず relay が関与できない) ため注入する。coder 側の報告経路は
   relay が機械的に確保するので注入しない。
   もう 1 つの理由: ダイアログ表示中に届いた send の本文はダイアログに吸われて消え、
   しかも herdr が working と分類するため送信側には成功が返る (実機で確認)。

   leader が既存セッション (すでに動いている claude) の場合、send.sh の使い方は
   verify-link のブリーフィングで届くが、規約ファイル本体は読んでいない。規約が
   届いていないセッションは codex exec など自前の手段に逃げる (実機で確認) ため、
   規約ファイルを読ませるか依頼文にプラグインの使用を明示する。

## 送信

```bash
bash ~/.config/herdr/plugins/github/thkt.agentchat-*/actions/send.sh coder "POST /todos を実装して。完了したら send で報告して。"
```

exit code の意味は `actions/send.sh` 冒頭のコメントと `templates/CLAUDE.md` を参照。

## 暴走防止

- 自発的な起こしはしない。起こしは常に send (明示的な送信) が起点。状態監視 (`events/log-status.sh`) は観測専用。
- クールダウン: 同一宛先へ同一内容を 30 秒以内に再送しない。
- 往復上限: 同一宛先へ 10 分間に 30 送信を超えたら停止し、人間への報告を促す。

## 受け入れテスト台本

実機で上から順に実行し、すべて満たせば合格。

1. セットアップ手順で `leader` / `coder` の 2 ペインを立ち上げる。
2. `herdr plugin link ./` → `herdr plugin list` で有効を確認する。
3. leader にタスクを依頼する。
   例:「Node.js と Express で、メモリ保存の TODO API を作りたい。POST /todos、GET /todos、
   PATCH /todos/:id/complete の 3 つ。npm test も付けて。計画を立てて、実装は send で
   coder に依頼して、完成まで面倒を見て。」
4. 自動着手: leader の `send coder "..."` で、idle の coder が人手なしに working になる。
5. 往復: coder が `send leader "..."` で質問し、leader の回答で再開する。
6. 報告: coder の完了報告を leader が受け、人間に報告する。
7. 暴走なし: 往復が上限内で収束し、無限に起こし合っていない。
8. (該当時) coder の応答が `herdr agent read <名前>` で読めない場面が出たら、
   `--source visible` で読む。それでも読めなければ「応答を Markdown で一時ファイルに
   書き、パスだけ返す」フォールバックに切り替える。

## 完了の自動中継 (pending 機構)

「完了したら send で報告して」という LLM への指示は破られうる (実機で 2 回再発)。そのため
報告経路は herdr の状態遷移から機械的に確保する。send は宛先への送信成功時に返信待ち
マーカーを記録し、宛先のターン完了 (done / idle) をイベントフック (`events/relay.sh`) が
検知すると、宛先ペインの直近出力を返信先へ自動中継してマーカーを消す。受信者が send での
報告を怠っても報告が届き、send した場合は中継が重複 1 通になるだけで済む。中継は 1 送信に
つき最大 1 回で、中継自体はマーカーを書かないため起こし合いのループにはならない。

人間との境界も機構化している。blocked 遷移は承認という人間の仕事なので、agent への send
でなく `notification show` の toast で人間へ直接通知する (マーカーは保持し、承認後の
ターン完了で通常の中継が走る)。返信先が実在しない宛先 (`--reply-to human` など) の場合も
完了 toast にフォールバックするため、leader を介さない直接依頼 (human → coder) も
「送って待つだけ」で完結する。

## 実機検証で確認した注意点 (herdr 0.7.5)

- `agent start` 直後の send は、claude の初期化 (SessionStart hook 実行中) に本文ごと消えることがある。
  最初の送信の前に短い疎通メッセージを 1 往復させると安全になる。
- `agent_prompt_stalled` (exit 6) は「未達」を意味しない。本文は宛先の入力欄に投入済みで、宛先の UI 状態
  により Enter だけが呑まれた場合がある。そのため send.sh は stalled 時に Enter を追い打ちして送信を
  完成させる。同一内容の再送は二重投入になるため行わない。
- codex の承認 UI (コマンド実行確認) は herdr が blocked と検知し、send.sh の blocked ガードが機能する。
  承認は人間が `herdr agent send-keys coder <key>` で行う。承認なし運用にするなら
  `codex -a never -s workspace-write -c sandbox_workspace_write.network_access=true` で起動する
  (network 許可がないと supertest 等のテスト用 listen が sandbox に拒否される)。
- exit 7 (着手観測タイムアウト) は宛先がすでに working のときにも出る。この場合、本文は届いており
  宛先のキューで処理される。`herdr agent get <名前>` で working を確認できたら再送しない。
- sandbox 下の送信者 (codex workspace-write 等) は状態ディレクトリに書けないことがある。send.sh は
  警告を出してガード記録なしで送信を続行する。ガードを生かすには `HERDR_PLUGIN_STATE_DIR` に
  書き込み可能なパスを指定して呼び出す。

## 非目標

pane title への返信待ち表示は現状維持する。それ以外の常設状態 UI、会話履歴の完全な永続化、
ペイン喪失後の文脈復元、3 体以上の汎用構成、放置運転 (夜間仕込み) はスコープ外。
